"""Evaluator for Course 2 ch3 class 1 — EWC regularization.

Usage:
  python eval.py --config configs/default.yaml
  python eval.py --config configs/default.yaml --checkpoint /path/to/checkpoint

If --checkpoint points to an existing directory, loads the model from it and
evaluates on all task eval splits. If absent, falls back to re-running train.py
with a warning.
"""
...
def main() -> None:
    ...
    log = get_logger("course2.ch3.eval")
