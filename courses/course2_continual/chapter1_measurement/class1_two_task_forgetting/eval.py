"""Evaluator for Course 2 ch1 class 1 — two-task forgetting measurement.

Usage:
  python eval.py --config configs/default.yaml
  python eval.py --config configs/default.yaml --checkpoint /path/to/checkpoint

If --checkpoint points to an existing directory the model is loaded from it
and evaluated on all task eval splits. If the checkpoint is absent, falls back
to re-running train.py (re-trains and re-evaluates), with a warning.
"""
...
def main() -> None:
    ...
    log = get_logger("course2.ch1.eval")
    ...
    if ckpt and ckpt.exists():
        ...
        model = AutoModelForSequenceClassification.from_pretrained(
            str(ckpt), num_labels=cfg["num_labels"]
        )
        model.eval()
