...
def main() -> None:
    ...
    enc_metrics = _encoder_step(cfg["encoder_backbone"], prompt, log)
    _emit(cfg, cfg["encoder_backbone"], enc_metrics)

    dec_metrics = _decoder_step(cfg["decoder_backbone"], prompt, cfg["generation"], log)
...
def _encoder_step(name: str, prompt: str, log) -> dict[str, float]:
    ...
    emb = bb.model.encode(prompt, convert_to_tensor=True) if bb.kind == "sentence-encoder" \
        else _encoder_pool(bb, prompt)
    dim = int(emb.shape[-1])
    log.info("[encoder] dim=%d hidden_size=%d norm=%.4f", dim, bb.hidden_size, float(emb.norm()))
    return {
...
def _decoder_step(name: str, prompt: str, gen_cfg: dict, log) -> dict[str, float]:
    import torch
    ...
    log.info("[decoder] new_tokens=%d preview=%s", new_tokens, text[:120].replace("\n", " "))
    return {
